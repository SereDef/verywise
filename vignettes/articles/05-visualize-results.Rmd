...
All done with the analyses? That was a handful, you must be hungry for some brain maps.
...
- `plot_vw_diff()` for plotting a difference between two brain surface maps. You can use this to check whether two terms have similar spatial mapping for example, or to compare the fit of two models (see the [model comparison article](07-model-fit-comparison.html)). 
...
The most common function used is `plot_vw_map()`. It takes a `verywise` results directory, locates the coefficient map for
a term of interest, optionally applies a threshold, and renders the result on a brain surface either interactively (HTML) or as a PNG. 
...
- **`"cws"`**: cluster-wise significant vertices (default), assuming cluster correction was computed during the
  analysis.
- **`"fdr<0.05"`**: an FDR-corrected significance level, assuming FDR-adjusted p-values were calculated at
  the analysis stage. Note, use whatever threshold you like (e.g. `"fdr<0.001"`), we'll do the rest. 
- **A numeric value**, e.g. `0.001`: interpreted as a raw coefficient (absolute) threshold

In practice, `"cws"` is often the most interpretable option for final figures, while numeric thresholds can be useful during early
exploration.
